...
const CONFIG = {
  ...
  IS_DEV: process.env.NODE_ENV?.includes('dev'),
...
axios.defaults.headers.common['Accept-Language'] = 'zh-CN,zh;q=0.9,en-US;q=0.8,en;';
axios.defaults.headers.common['User-Agent'] = 'Mozilla/5.0 (X11; Linux x86_64; rv:12.0) Gecko/20100101 Firefox/21.0';
...
app.get('/download-pdf/*', asyncHandler(async (req, res) => {
...
app.get('/download-app/*', asyncHandler(async (req, res) => {
  const rawPath = req.params[0];
...
app.get('/tweet-page-7', asyncHandler(async (req, res) => {
  try {
    const result = [];
...
app.get('/tweet-page', asyncHandler(async (req, res) => {
...
app.get('/search-tweet-page', asyncHandler(async (req, res) => {
  ...
  try {
    const searchUrl = `https://api.github.com/search/code?q=${encodeURIComponent(keyword)} in:file repo:hello-world-1989/json`;
    const searchResponse = await makeRequest(searchUrl, {
      headers: { Authorization: `Bearer ${CONFIG.GITHUB_TOKEN}` }
    });

    const promises = searchResponse?.data?.items
...
app.get('/news-page', asyncHandler(async (req, res) => {
...
app.get('/github', asyncHandler(async (req, res) => {
  try {
    const response = await makeRequest('https://raw.githubusercontent.com/hello-world-1989/cn-news/main/server.txt');
...
app.get('/ss-key', asyncHandler(async (req, res) => {
  try {
...
app.get('/youtube', createResourceEndpoint('https://raw.githubusercontent.com/hello-world-1989/accessible/main', 'youtube'));
app.get('/obfs4', createResourceEndpoint('https://raw.githubusercontent.com/hello-world-1989/cn-news/main', 'obfs4'));
app.get('/wiki', createResourceEndpoint('https://raw.githubusercontent.com/hello-world-1989/accessible/main', 'wiki'));
app.get('/nitter', createResourceEndpoint('https://raw.githubusercontent.com/hello-world-1989/accessible/main', 'nitter'));
app.get('/searchx', createResourceEndpoint('https://raw.githubusercontent.com/hello-world-1989/accessible/main', 'searchx'));
app.get('/pdf', createResourceEndpoint('https://raw.githubusercontent.com/hello-world-1989/whyyoutouzhele/main', 'pdf'));

app.get('/host', asyncHandler(async (req, res) => {
  try {
...
app.get('/apple-account', asyncHandler(async (req, res) => {
  try {
    let account;
...
app.get('/news-data', asyncHandler(async (req, res) => {
...
app.get('/tweet', asyncHandler(async (req, res) => {
  const { year, month, day, endDay, id } = req.query;
    ...
    if (month && day && endDay) {
      ...
      for (let i = startDayNumber + 1; i <= endDayNumber; i++) {
        ...
        try {
          ...
          if (rawCurrentData) {
            const resultStr = Buffer.from(rawCurrentData, 'base64').toString('utf-8');
...
app.get('/event', validateQueryParams(['year']), asyncHandler(async (req, res) => {
...
app.get('/search-tweet', validateQueryParams(['keyword']), asyncHandler(async (req, res) => {
  const { keyword } = req.query;
  ...
  try {
    ...
    const results = await Promise.all(promises);
    const allTweets = results.map(item => {
...
app.get('/news-resource/*', asyncHandler(async (req, res) => {
  const rawPath = req.params[0];
...
app.get('/node', asyncHandler(async (req, res) => {
...
app.get('/renew-plan', validateQueryParams(['token']), asyncHandler(async (req, res) => {
...
app.get('/check-status', asyncHandler(async (req, res) => {
  try {
...
app.get('/ip-check', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res.send({ ip, port, status: false, timestamp: new Date().toISOString() });
  }
}));

app.get('/url-check/*', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res.send({ url: rawURL, status: false, timestamp: new Date().toISOString() });
  }
}));
...
async function report() {
  ...
  try {
    ...
    await makeRequest(`https://end-gfw.com/node?ip=${ip}&port=${CONFIG.NODE_PORT}`);
...
async function periodicCheckConnection() {
  ...
  const sortedHosts = hosts.sort((a, b) => (a.updatedTime > b.updatedTime ? -1 : 1));
...
async function fetchAPI() {
  ...
  try {
    const url = 'https://api.github.com/repos/hello-world-1989/cn-news/contents/end-gfw-together-ss';
    const response = await makeRequest(url, {
      headers: { Authorization: `token ${CONFIG.GITHUB_TOKEN}` }
    });
